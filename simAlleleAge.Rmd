...
// [[Rcpp::export]]
List getAlleleAgecpp(float gamma, int N, int ndraws){
    int gen, lky;
    int i = 0;
    double Xt = 0.5/N;
    double mu = 0.0;
    ...
    for(gen=1; gen<50000; gen++){
        ...
        if(Xt < 0.5/N || Xt > (N-0.5/N)){
            lky = Rcpp::RcppArmadillo::sample(gen-1,1);
            age[i] = sim[lky];
            Xl[i] = lky;
            goto restart;
        }
...
for(n in 1:length(gamma)){
    newfinalXl[((n-1)*1000+1):(n*1000),c(3,1)]<-getAlleleAgecpp(gamma[n], N, 1000)
}
```
