...
estimXl<-function(Xl, gamma, ndraws){
    ...
    for(n in 1:length(gamma)){
        ...
        FXl<-(expint_Ei(2*gamma[n]*(Xl-1)) - exp(-2*gamma[n])*expint_Ei(2*gamma[n]*Xl) - log(1-Xl) + log(Xl) - lbscal)/scal.fact # exp(2*gamma[n])/(exp(2*gamma[n])-1) gets cancelled out from num and den...
        
        u<-runif(ndraws, 0, 1)
        finalXl[((n-1)*ndraws+1):(n*ndraws)]<-Xl[sapply(u,function(x){which.min(abs(FXl-x))})]
    }
    return(finalXl)
}
