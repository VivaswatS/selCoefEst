...
estimXl<-function(Xl, gamma, ndraws){
    ...
    for(n in 1:length(gamma)){
        ...
        pXl<-(expint_Ei(gamma[n]*(Xl-1)) - exp(-gamma[n])*expint_Ei(gamma[n]*Xl) - log(1-Xl) + log(Xl) - lbscal)/scal.fact #does this need to be here: /(1-exp(-gamma[n]))?
...
```{r, fig.cap="From 1/2N to p"}
knitr::include_graphics("~/Downloads/12ntop.png")
```

```{r fig.cap="From}
knitr::include_graphics("~/Downloads/pto12n.png")
```
...
exp.al<-function(p, gamma, N){
    ...
    return(-2*(term1-term2)/(gamma*expm1(gamma)) + 2*expm1(gamma*p)*(term3-term4)/(gamma*expm1(gamma)*(exp(gamma)-exp(gamma*p))))
}
...
plot(log(finalXl[,5]),log(finalXl[,4]),pch=20,col="grey40",xlab="Mean of simulated",ylab="Expected",main="Allele age")
```
...
*(correlation is very low, need to recheck this...)*

```{r}
final.data<-cbind(final.data, apply(final.data, 1, function(x){exp.al(x[1], x[2], N)}))
colnames(final.data)[4]<-"exp.al"
final.data[,4]<-final.data[,4]*2*N
plot(final.data[,4],finalXl[,3],pch=20,col="grey40",ylab="Simulated",xlab="Expected",main="Allele age")
```
...
```{r}
plot(sort(newfinalXl[newfinalXl$gamma==gamma[7],1]), sort(finalXl[finalXl$gamma==gamma[7],1]), col='grey', main=paste0("Xl, gamma=",gamma[7]), ylab="backward approx", xlab="FIT sims")

plot(sort(newfinalXl[newfinalXl$gamma==gamma[7],3]), sort(finalXl[finalXl$gamma==gamma[7],3]), col='grey', main=paste0("al, gamma=",gamma[7]), ylab="backward approx", xlab="FIT sims")
```


```{r}
plot(sort(newfinalXl[newfinalXl$gamma==gamma[17],1]), sort(finalXl[finalXl$gamma==gamma[17],1]), col='grey', main=paste0("Xl, gamma=",gamma[17]), ylab="backward approx", xlab="FIT sims")

plot(sort(newfinalXl[newfinalXl$gamma==gamma[17],3]), sort(finalXl[finalXl$gamma==gamma[17],3]), col='grey', main=paste0("al, gamma=",gamma[17]), ylab="backward approx", xlab="FIT sims")
```

```{r}
hist(finalXl[finalXl$Xl>0.1&finalXl$Xl<0.2,3])
hist(newfinalXl[newfinalXl$Xl>0.1&newfinalXl$Xl<0.2,3],col='grey')
```
