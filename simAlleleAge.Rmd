...
```{r}
# Bustamante et al 2001 formulation for haploid effective pop size
gamma<-101
x<-seq(0,1,0.01)
pt <- 1/(x*(1-x))*(1-exp(-gamma*(1-x)))/(1-exp(-gamma))
hist(pt)
...
Here, I will use the $X_0$ as the starting frequency and run WF sims until it reaches a frequency of $<1/2N$ -- record this generation as the allele age. 

```{r}
alleleAge<-function(X_0, gamma, N){
