...
while(cnt<200){
    ...
    if(length(Xl)<1850){
        ...
        cnt<-cnt+1
        finalXl<-c(finalXl,Xl[1:1850])
    }
}

# storing the present day allele frequencies for use in binom calcs
write.table(finalXl[seq(0,length(finalXl),1850)],'PReFerSims/msselfiles/hapfiles/Freq-22.0.txt',sep='\n',quote=F,col.names=F,row.names=F,append=T)
 
## need to run this in the Console below for some reason
# plot(finalXl[1:1850],col='#1f77b4',type='l',xlab='Generations',ylab='Allele Frequency',log='xy')
# for(i in 2:50){
#     lines(finalXl[((i-1)*1850+1):(i*1850)],col='#1f77b4')
#     lines(finalXl22[((i-1)*1850+1):(i*1850)],col='#ff7f0e')
# }
# legend('topleft',c('200','20'),col=c('#1f77b4','#ff7f0e'),lty=1)
...
for(i in 1:200){
    ...
    write(paste0('# s: 0 age: ',1850/40000),'PReFerSims/msselfiles/TrajMsselLike-22.0.txt',append=T)
    write.table(rbind(c(0,0),cbind(1:1850/40000,rev(finalXl[((i-1)*1850+1):(i*1850)]))),'PReFerSims/msselfiles/TrajMsselLike-22.0.txt',append=T,sep=' ',quote=F,col.names=F,row.names=F)

    system("cat PReFerSims/msselfiles/TrajMsselLike-22.0.txt | ~/mssel/stepftn > PReFerSims/msselfiles/CurrTraj-22.0.txt")
    for(c in 1:10){
        system(paste0("~/mssel/mssel3 3 1 1 2 PReFerSims/msselfiles/CurrTraj-220.0.txt 500 -r 0 1000 -t 20 -T > PReFerSims/msselfiles/hapfiles/haps-22.0_",i,"_c",c,"_t1850.ms"))
    }
}
```
