...
$$
\bar t^{**}(x;p) = \int_{1/2N}^{p} 2\{e^{\gamma x}-1 \} \{e^{\gamma (1-x)}-1\}[\gamma x(1-x) (e^\gamma-1)]^{-1} dx + \\ \int_p^{1-1/2N} 2e^{-\gamma(1-x)} \{1-e^{-\gamma p}\}\{e^{\gamma(1-x)}-1\}^2 \times[\gamma x(1-x)(1-e^{-\gamma})(e^{\gamma(1-p)}-1)]^{-1}dx
$$
...
finalXl$exp.al<-apply(finalXl, 1, function(x){exp.al(x[1], x[2], N)})
finalXl$exp.al<-finalXl$exp.al*2*N

plot(finalXl$al,finalXl$exp.al,pch=20,col="grey40",xlab="Simulated",ylab="Expected",main="Allele age")
```