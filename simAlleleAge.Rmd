...
alleleTrajFwd<-function(gamma, N){
    ...
    # while(Xt[gen]>=0.5/N && Xt[gen]<1-0.5/N){
    #     gen<-gen + 1
    #     
    #     mu<-Xt[gen-1]+0.25*gamma*Xt[gen-1]*(1-Xt[gen-1])/tanh(gamma*(1-Xt[gen-1]))/N
    #     Xt[gen]<-rnorm(1, mean=mu, sd=sqrt(Xt[gen-1]*(1-Xt[gen-1]))*0.5/N)
...
write.table(matrix(finalXl,nrow=1000,ncol=600,byrow=T),'PReFerSims/msselfiles/hapfiles/allFreq-220.0.txt',sep=',',quote=F,col.names=F,row.names=F)
  
# ## need to run this in the Console below for some reason
plot(finalXl[1:600],col='#2ca02c',type='l',xlab='Generations',ylab='Allele Frequency',ylim=c(0,1))
for(i in 2:1000){
    lines(finalXl[((i-1)*600+1):(i*600)],col='#2ca02c')
...
for(i in 401:1000){
    write("// This is an input file for Hudson's mssel stepftn.c program",'PReFerSims/msselfiles/TrajMsselLike-220.0.txt')
    write('1 N0: 10000','PReFerSims/msselfiles/TrajMsselLike-220.0.txt',append=T)
    write(paste0('# s: ',220/40000,' age: ',600/40000),'PReFerSims/msselfiles/TrajMsselLike-220.0.txt',append=T)
    write.table(rbind(c(0,0),cbind(1:600/40000,finalXl[((i-1)*600+1):(i*600)])),'PReFerSims/msselfiles/TrajMsselLike-220.0.txt',append=T,sep=' ',quote=F,col.names=F,row.names=F)

    system("cat PReFerSims/msselfiles/TrajMsselLike-220.0.txt | ~/mssel/stepftn > PReFerSims/msselfiles/CurrTraj-220.0.txt")
    for(c in 1:10){
        system(paste0("~/mssel/mssel3 3 1 1 2 PReFerSims/msselfiles/CurrTraj-220.0.txt 500 -r 0 1000 -t 20 -T > PReFerSims/msselfiles/hapfiles/haps-220.0_",i,"_c",c,"_t600.ms"))
    }
}
```
...
```{r}
Xl <- alleleTrajFwd(300,10000)
write("// This is an input file for Hudson's mssel stepftn.c program",'PReFerSims/msselfiles/TrajMsselLike-220.0.txt')
write('1 N0: 10000','PReFerSims/msselfiles/TrajMsselLike-220.0.txt',append=T)
write(paste0('# s: ',220/40000,' age: ',600/40000),'PReFerSims/msselfiles/TrajMsselLike-220.0.txt',append=T)
write.table(rbind(c(0,0),cbind(1:600/40000,finalXl[((12-1)*600+1):(12*600)])),'PReFerSims/msselfiles/TrajMsselLike-220.0.txt',append=T,sep=' ',quote=F,col.names=F,row.names=F)
system("cat PReFerSims/msselfiles/TrajMsselLike-220.0.txt | ~/mssel/stepftn > PReFerSims/msselfiles/CurrTraj-220.0.txt")
der<-rbinom(1,40,finalXl[12*600])
system(paste0("~/mssel/mssel3 40 1 ",40-der," ",der," PReFerSims/msselfiles/CurrTraj-220.0.txt 500 -r 0 1000 -t 20 -T > PReFerSims/msselfiles/hapfiles/haps-220.0_123_t600.ms"))
```
