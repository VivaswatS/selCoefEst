...
finalest2 <- read.table('resfiles/finalRes2024-11-01.csv',header=T,sep=',')
...
ta <- t(matrix(finalest2$trueage,nrow=20,ncol=50))
ta[,1] <- -abs(ta[,1])
ta0 <- ta[,20]
fo <- t(matrix(finalest2$freqonly,nrow=20,ncol=50))
fo0 <- fo[,20]
rm <- t(matrix(finalest2$relmid,nrow=20,ncol=50))
ims <- t(matrix(finalest2$impsamp,nrow=20,ncol=50))
avg <- t(matrix(finalest2$simpavg,nrow=20,ncol=100))

for(idx in 10:14){
  ta[which(ta[,idx] < -100),idx] <- NA
}

# all freq. for branch averaging scheme
allfreq <- colVar(fo)/colVar(ta)

df2 <- data.frame(gam=c(big.gamma,list.of.gamma[1:40]),
                  method=c(rep('Using all freq.\n(true ages, n = 100)',50),rep('Using all freq.\n(true ages, n = 10)',20),rep('Using only MAF>2.5%\n(Relate est. ages with branch avg.)',20)),
                  val=c(ratio.sd,allfreq,colVar(fo)/colVar(ims)))

df2 %>% dplyr::filter(method %in% c('Using all freq.\n(true ages, n = 100)','Using all freq.\n(true ages, n = 10)')) %>%
ggplot() + geom_point(aes(x=gam,y=val,color=method,fill=method)) + theme_bw() +
    ...
    scale_x_continuous(trans='pseudo_log',breaks=c(0,-1,-10,-100),labels=c(0,-1,-10,-100)) + 
  scale_color_manual(values=c('grey70','grey20','grey80')) + scale_fill_manual(values=c('grey70','grey20','grey80')) +
  ...
  # scale_y_continuous(trans='pseudo_log',breaks=c(100, 30, 10, 3, 1, 0),labels=c(100, 30, 10, 3, 1, 0)) +
  geom_hline(yintercept=1,linetype="dashed", color = "grey50", size=2) +
  geom_vline(xintercept=-10,linetype='dashed',color='dodgerblue4',size=1.5)
ggsave('~/selCoefEst/figs/RatioSDcontantRel.pdf',height=5,width=7)
...
df4<-data.frame(tid=rep(rep(1:20,each=50),4),
                sim=rep(rep(list.of.gamma[1:20],each=50),4),
                method=c(rep('Freq only',1000),rep('Freq & age with true ages',1000),rep('Freq & age with Relate est.',1000),rep('Freq & age with branch avg.',1000)),
...
dplyr::filter(method %in% c('Freq only','Freq & age with true ages')) %>%
 # dplyr::filter(method %in% c('Freq only','Freq & age with true ages')) %>%
 dplyr::mutate(xpos=rep(rep(c(1,6,11,14.5,18,20),each=50),2)) %>% ggplot(aes(x=xpos,y=est,group=interaction(factor(tid),method))) + geom_violin(aes(x=xpos,y=est,fill=method),position='dodge',trim=T,scale='width') +
  ...
  scale_fill_manual(values=c('grey40','goldenrod')) + 
    ...
    theme_bw() + theme(axis.text = element_text(size = rel(2)),
                       axis.title = element_text(size = rel(2)),
...
ggsave('~/selCoefEst/figs/EstNegwIS.pdf',height=7,width=9)

# c(1,5,9,13,17,19)
df4 %>% dplyr::filter(tid %in% c(1,5,9,13,17,19)) %>% dplyr::filter(method %in% c('Freq only','Freq & age with true ages','Freq & age with Relate est.','Freq & age with branch avg.')) %>% dplyr::mutate(xpos=rep(rep(c(1,6,11,14.5,18,20),each=100),4)) %>% ggplot(aes(x=xpos,y=est,group=interaction(factor(tid),method))) + geom_violin(aes(x=xpos,y=est,fill=method),position='dodge',trim=T,scale='width') +
    # stat_summary(aes(x=factor(tid),y=est,fill=method),fun.data=mean_sdl, geom="pointrange", color="black",position=position_dodge(width = 1.9),size=0.1) +
    geom_hline(aes(yintercept=sim),color='grey50',lty=2,size=0.5) +
    # geom_hline(aes(yintercept=c(0,-1,-3,-10)),color='grey50',lty=2,size=0.5) +
    labs(x=TeX('$\\gamma$'), y = TeX('$\\hat{\\gamma}$')) +
  scale_fill_manual(values=c('grey80','steelblue','grey40','goldenrod')) + 
    ...
    theme_bw() + theme(axis.text = element_text(size = rel(2)),
                       axis.title = element_text(size = rel(2)),
                       legend.key = element_rect(colour="transparent",size=rel(1.5)),
                       legend.text = element_text(size=rel(1.5)),
                       legend.title = element_text(size=rel(1.5)),
                       legend.background = element_rect(fill='transparent'),
                       legend.position=c(0.7,0.25))
ggsave('~/selCoefEst/figs/EstNeg2.5pcwIS.pdf',height=7,width=9)

# simple averaging scheme 
df4<-data.frame(tid=rep(rep(1:20,each=100),5),
                sim=rep(rep(list.of.gamma[1:20],each=100),5),
                method=c(rep('Freq only',2000),rep('Freq & age with true ages',2000),rep('Freq & age with Relate est.',2000),rep('Freq & age with reweighting',2000),rep('Freq & age with branch avg.',2000)),
                est=c(as.vector(fo),as.vector(ta),as.vector(rm),as.vector(ims),as.vector(avg)))


df4 %>% dplyr::filter(tid %in% c(1,5,9,13,17,19)) %>% #dplyr::filter(method %in% c('Freq only','Freq & age with true ages','Freq & age with reweighting','Freq & age with branch avg.','Freq & age with Relate est.')) %>% 
  dplyr::mutate(xpos=rep(rep(c(1,6,11,14.5,18,20),each=100),5)) %>% 
  ggplot(aes(x=xpos,y=est,group=interaction(factor(tid),method))) + geom_violin(aes(x=xpos,y=est,fill=method),position='dodge',trim=F,scale='width') +
    # stat_summary(aes(x=factor(tid),y=est,fill=method),fun.data=mean_sdl, geom="pointrange", color="black",position=position_dodge(width = 1.9),size=0.1) +
    geom_hline(aes(yintercept=sim),color='grey50',lty=2,size=0.5) + 
    labs(x=TeX('$\\gamma$'), y = TeX('$\\hat{\\gamma}$')) +
  scale_fill_manual(values=c('lightskyblue3','steelblue','grey80','grey40','goldenrod')) + 
    scale_y_continuous(trans='pseudo_log',breaks=c(1,0,-1,-10,-100),labels=c(1,0,-1,-10,-100)) + 
    scale_x_discrete(breaks=c(1,11,18,20),labels=c(-100,-10,-1,0),limits=1:20) +
...
### Remaking figure 5 but with results from _n=10_ 

```{r}
finalest10 <- read.table('resfiles/finalRes2024-10-07.csv',header=T,sep=',')
finalest <- read.table('resfiles/finalRes2024-11-01.csv',header=T,sep=',')

ta <- t(matrix(finalest$trueage,nrow=20,ncol=50))
fo <- t(matrix(finalest$freqonly,nrow=20,ncol=50))

ta10 <- t(matrix(finalest10$trueage,nrow=20,ncol=50))
fo10 <- t(matrix(finalest10$freqonly,nrow=20,ncol=50))

# all freq. for branch averaging scheme
allfreq <- colVar(fo)/colVar(ta)
allfreq10 <- colVar(fo10)/colVar(ta10)

df2 <- data.frame(gam=c(big.gamma,list.of.gamma[1:40]),
                  method=c(rep('Using all freq.\n(true ages, n = 100)',50),rep('Using all freq.\n(true ages, n = 10)',20),rep('Using only MAF>2.5%\n(true ages, n = 100)',20)),
                  val=c(ratio.sd,allfreq10,allfreq))

df2 %>% dplyr::filter(method %in% c('Using all freq.\n(true ages, n = 100)','Using all freq.\n(true ages, n = 10)','Using only MAF>2.5%\n(true ages, n = 100)')) %>%
ggplot() + geom_point(aes(x=gam,y=val,color=method,fill=method)) + theme_bw() +
    geom_smooth(aes(x=gam,y=val,color=method,fill=method),method='loess',) +
    scale_x_continuous(trans='pseudo_log',breaks=c(0,-1,-10,-100),labels=c(0,-1,-10,-100)) + 
  scale_color_manual(values=c('grey80','grey20','grey50')) + scale_fill_manual(values=c('grey80','grey20','grey50')) +
    labs(x=TeX('$\\gamma$'),y='ratio of Var') +
    guides(color = guide_legend(override.aes = list(fill = "white"))) +
    theme(axis.text = element_text(size=rel(2)),
          axis.title = element_text(size=rel(2)),
  ...
  # scale_y_continuous(trans='pseudo_log',breaks=c(100, 30, 10, 3, 1, 0),labels=c(100, 30, 10, 3, 1, 0)) +
  geom_hline(yintercept=1,linetype="dashed", color = "grey50", size=2) +
  geom_vline(xintercept=-10,linetype='dashed',color='dodgerblue4',size=1.2) + 
...
df4 %>% dplyr::filter(tid %in% c(4,8,12,15,20)) %>% dplyr::filter(method %in% c('Freq only','Freq & age with true ages')) %>% dplyr::mutate(xpos=rep(rep(c(3,6.5,10,15,20),each=100),2)) %>% ggplot(aes(x=xpos,y=est,group=interaction(factor(tid),method))) + geom_violin(aes(x=xpos,y=est,fill=method),position='dodge',trim=T,scale='width') +
  ...
  scale_fill_manual(values=c('grey40','goldenrod')) + 
    ...
    theme_bw() + theme(axis.text = element_text(size=rel(2)),
                       ...
                       legend.text = element_text(size=rel(1.5)),
                       legend.title = element_text(size=rel(1.5)))
ggsave('~/selCoefEst/figs/EstPos.pdf',height=7,width=9)
...
Exp$TVD <- c(0.21907112136788967, 0.19435652823943708, 0.1727109397699595, 0.1538373684569866, 0.13739062547528058, 0.12301095847989185, 0.11034638623042936, 0.0990659514744748, 0.08886370325947851, 0.07945495512600595, 0.07056819507185265, 0.06193144362629964, 0.05325791423048176, 0.044290383804681435, 0.035039869903120566, 0.02655161998784865, 0.019751076095418263, 0.014296532379361668, 0.0100818104872773, 0.006939055355446436, 0.004673069984906324, 0.002615934986792208, 0.001309582132743911, 0.0005682713911570202, 0.00020414616196510725, 6.092934374303112e-05, 1.6122671519604116e-05, 4.691492336220921e-06, 8.628237992222748e-07, 5.932478368487888e-08, 7.937887679901296e-07, 6.379232643599702e-06, 2.7739239140083712e-05, 0.0001007376338771726, 0.0003043550195369986, 0.0007860673371383556, 0.0017953188980250048, 0.0037151461987717438, 0.007081475804341278, 0.011237470009193066, 0.017724165598232075, 0.027655827872761822, 0.04248275637664964, 0.06394991611715076, 0.09385517678266453, 0.13327032396443378, 0.18118505109182825, 0.23406800080204807, 0.287719178126543, 0.33906932090292435, 0.3863481578904703, 0.42892639962816737, 0.46710785295174156, 0.5016896819401333, 0.5335018976252779, 0.5631744423142697, 0.5911336353343251, 0.6176690112902996, 0.642984905384683)
...
df <- data.frame(gam=c(big.gamma[as.integer(seq(1,49,length.out=22))],unique(-finalest2[,2]),Exp$gamma),
                 rsd=c(ratio.sd[as.integer(seq(1,49,length.out=22))],ratio.var.pos,Exp$KLdiv+Exp$KLdivrev),
                 method=c(rep('Using true ages',42),rep('KL divergence',59)))

ggplot(df) + 
  # geom_point(data=subset(df,method == 'Using true ages'),aes(x=gam,y=rsd,color=method,fill=method),size=3,show.legend=F) +
  theme_bw() + 
  geom_line(data=subset(df,method %in% c('KL divergence')),aes(x=gam,y=rsd,color=method),linewidth=2) +
  scale_color_manual(values=c('firebrick')) +  scale_fill_manual(values=c('grey60','grey20')) + 
    # geom_smooth(data=subset(df,method != 'TVD'),aes(x=gam,y=rsd,color=method,fill=method),method='loess',size=2,show.legend=F) +
  ...
  guides(color = guide_legend(override.aes = list(fill = "white"))) + 
    labs(x=TeX('$\\gamma$'),y=TeX('Kl divergence')) + #scale_y_continuous(trans='log10') +
  theme(axis.text=element_text(size=rel(2)),
        axis.title=element_text(size=rel(2)),
        legend.position="none")#c(0.25,0.8),
ggsave('~/selCoefEst/figs/gaininfo_onlytvd.pdf',height=7,width=9)
```
...
ggplot(timings, aes(x = Amax, y = time, color = as.factor(twon), linetype = method)) +
  ...
  labs(x = expression(T[max]), y = "Time (s)", color = "2n", linetype = "Method") + # Axis and legend labels
...
ggplot(aes(x = twon, y = time, color = as.factor(Amax), linetype = method)) +
  ...
  scale_y_log10() + # Log scale for y-axis
  scale_color_grey(start = 0.8, end = 0.2) + # Greyscale colors for `twon`
  labs(x = "2n", y = "Time (s)", color = expression(T[max]), linetype = "Method") + # Axis and legend labels
  scale_linetype_manual(values = c("dashed", "dotted", "solid")) + 
  theme_bw() + # Minimal theme
  theme(legend.position = "right", # Position the legend
        legend.title = element_text(face = "bold"), # Bold legend title
...
#### Timings for genealogy calculations

```{r}
times <- read.table("~/selCoefEst/PReFerSims/resfiles/times.txt", header=T)

times %>% filter(haps==300) %>% mutate(timeA = time1 + time2 + time3, timeB = time1 + time2 + time4) %>% 
  ggplot() + aes(color = factor(Nc)) + theme_classic(base_size=20) + ylab('time (s)') +
  geom_line(aes(x = n, y = time3), linetype = "dotted", lwd=1) + geom_point(aes(x = n, y = time3)) +
  geom_line(aes(x = n, y = time2), linetype = "dashed", lwd=1) + geom_point(aes(x = n, y = time2)) +
  geom_line(aes(x = n, y = time1), linetype = "solid", lwd=1) + geom_point(aes(x = n, y = time1)) + 
  geom_line(aes(x = n, y = time4), linetype = "dotdash", lwd=1) + geom_point(aes(x = n, y = time1)) + scale_color_brewer(palette = 'Dark2') 

times %>% filter(haps==300) %>% mutate(timeA = time1 + time2 + time3, timeB = time1 + time2 + time4) %>% 
  ggplot() + aes(color = factor(Nc)) + theme_classic(base_size=20) + ylab('time (s)') +
  geom_line(aes(x = n, y = timeA), linetype = "dashed", lwd = 2) + 
  geom_line(aes(x = n, y = timeB), linetype = "solid", lwd = 2) + 
