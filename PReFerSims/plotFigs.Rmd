...
finalest2 <- read.table('resfiles/finalRes2024-07-18.csv',header=T,sep=',')
finalest2 <- read.table('resfiles/finalRes2024-07-18_2.5pc.csv',header=T,sep=',')
...
df2 <- data.frame(gam=c(big.gamma,list.of.gamma[1:40]),
                  method=c(rep('Using true ages (constant)',50),rep('Using all freq. (reweighting)',20),rep('Using Relate est. ages with branch avg.',20)),
                  val=c(ratio.sd,colVar(fo)/colVar(ims),colVar(fo)/colVar(avg)))

df2 %>% dplyr::filter(method %in% c('Using true ages (constant)','Using Relate est. ages with branch avg.')) %>%
ggplot() + geom_point(aes(x=gam,y=val,color=method,fill=method)) + theme_bw() +
    ...
    scale_x_continuous(trans='pseudo_log',breaks=c(0,-1,-10,-100),labels=c(0,-1,-10,-100)) + 
  scale_color_manual(values=c('grey70','grey20')) + scale_fill_manual(values=c('grey70','grey20')) +
...
ggsave('~/selCoefEst/figs/RatioSDcontantRel.jpeg',height=4,width=6)
...
df4<-data.frame(tid=rep(rep(1:20,each=100),4),
                ...
                est=c(as.vector(fo),as.vector(ta),as.vector(rm),as.vector(avg)))

# dplyr::filter(method %in% c('SFS',
df4 %>% dplyr::filter(tid %in% c(1,5,9,13,17,19)) %>% 
dplyr::filter(method %in% c('Freq only','Freq & age with true ages','Freq & age with Relate est.','Freq & age with branch avg.')) %>% dplyr::mutate(xpos=rep(rep(c(1,6,11,14.5,18,20),each=100),4)) %>% ggplot(aes(x=xpos,y=est,group=interaction(factor(tid),method))) + geom_violin(aes(x=xpos,y=est,fill=method),position='dodge',trim=T,scale='width') +
