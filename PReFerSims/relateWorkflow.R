...
for(al in alleles[,1]){
    system(paste0("sed -n '/^",al,"/p' MiniTest/Traj0.0.txt > MiniTest/TrajMini0.0.txt"))
...
for(al in alleles[,1]){
    ...
    rel.true[cnt] <- 80000-meta.prf[meta.prf[,5]==al,4]
    cnt<-cnt+1
}
 
plot(rel.true,0.5*(rel.est[,1]+rel.est[,2]),frame=F,log='xy',main=TeX('$\\gamma=0$'),
     ylab='est. age',xlab='true age (PReFerSim)',col='grey',pch=19)
points(rel.true,pmin(0.5*(rel.est[,1]+rel.est[,2]),2*rel.est[,1]),col='black',pch=20)
